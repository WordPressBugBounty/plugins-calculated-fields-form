	...
	$.extend(
		...
		$.fbuilder.controls['ffields'].prototype,
		{
			...
			show:function()
				{
					...
					return '<div class="fields '+cff_esc_attr(this.csslayout)+' '+this.name+' cff-textarea-field" id="field'+this.form_identifier+'-'+this.index+'" style="'+cff_esc_attr(this.getCSSComponent('container'))+'"><label for="'+this.name+'" style="'+cff_esc_attr(this.getCSSComponent('label'))+'">'+cff_sanitize(this.title, true)+''+((this.required)?"<span class='r'>*</span>":"")+'</label><div class="dfield"><textarea aria-label="'+cff_esc_attr(this.title)+'" '+((!/^\s*$/.test(this.rows)) ? 'rows='+cff_esc_attr(this.rows) : '')+' id="'+this.name+'" name="'+this.name+'"'+((this.minlength.length) ? ' minlength="'+cff_esc_attr(this.minlength)+'"' : '')+((this.maxlength.length) ? ' maxlength="'+cff_esc_attr(this.maxlength)+'"' : '')+' class="field '+cff_esc_attr(this.size)+((this.required)?" required":"")+'" '+((this.readonly)?'readonly':'')+' autocomplete="'+this.autocomplete+'" style="'+cff_esc_attr(this.getCSSComponent('textarea'))+'"></textarea>'+
			...
			after_show:function(){
				let e = $('[id="'+this.name+'"]'),
					pre = (!this.predefinedClick) ? String(this.predefined).replace( /<\s*\/?\s*textarea\b[^>]*>/gi, '' ) : '',
			...
			setVal:function( v, nochange )
				{
					let e 	= $( "[id='" + this.name + "']" ),
						bk 	= e.val();
					e.val( String(v).replace( /<\s*\/?\s*textarea\b[^>]*>/gi, '' ) );
