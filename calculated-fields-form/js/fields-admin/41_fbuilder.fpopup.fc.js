	...
	$.extend(
		...
		$.fbuilder.controls[ 'fcontainer' ].prototype,
		{
			...
			display:function( css_class )
				{
					css_class = css_class || '';
					return '<div data-control="'+this.ftype+'" class="fields '+this.name+((this.collapsed) ? ' collapsed' : '')+' '+this.ftype+' '+css_class+'" id="field'+this.form_identifier+'-'+this.index+'" title="'+this.controlLabel('Popup')+'" style="width:100%;"><div class="arrow ui-icon ui-icon-grip-dotted-vertical "></div>'+this.iconsContainer('<div title="Collapse (Ctrl+L)" class="collapse ui-icon ui-icon-folder-collapsed "></div><div title="Uncollapse (Ctrl+U)" class="uncollapse ui-icon ui-icon-folder-open "></div>')+
					...
					'</div>'+
					'<div class="fcontainer">'+'<span class="developer-note">'+cff_esc_attr(this._developerNotes)+'</span><label class="collapsed-label">Collapsed ['+this.name+']</label><div class="fieldscontainer"></div></div></div><div class="clearer"></div></div>';
				},
			...
			showPopupSettings:function()
				{
					...
					function getButtons( btn ) {
						...
						{
							if (items[i].ftype == 'fButton' && !$.fbuilder.inRepeater(items[i].name))
							{
