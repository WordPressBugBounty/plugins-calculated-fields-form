			...
			display:function( css_class )
				{
					...
					{
						str += '<div class="'+this.layout+'"><label><input disabled class="field disabled" type="checkbox" '+((this.choiceSelected[i])?"checked":"")+'/> '+cff_html_decode(this.choices[i])+'</label></div>';
					}
					return '<div class="fields '+this.name+' '+this.ftype+' '+css_class+'" id="field'+this.form_identifier+'-'+this.index+'" title="'+this.controlLabel('Checkboxes')+'"><div class="arrow ui-icon ui-icon-grip-dotted-vertical "></div>'+this.iconsContainer()+'<label>'+this.title+''+((this.required)?"*":"")+'</label><div class="dfield">'+this.showColumnIcon()+str+'<span class="uh">'+this.userhelp+'</span></div><div class="clearer"></div></div>';
				},
