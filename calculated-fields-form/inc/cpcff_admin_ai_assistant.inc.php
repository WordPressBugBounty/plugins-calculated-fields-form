...
if (! class_exists('CPCFF_AI_ASSISTANT')) {
	class CPCFF_AI_ASSISTANT
	{
		public static function dispatch()
		{
			if (isset($_POST['_cpcff_ai_assistant_nonce']) &&  current_user_can(apply_filters('cpcff_forms_edition_capability', 'manage_options'))) {
				...
				if (wp_verify_nonce($_cpcff_ai_assistant_nonce, 'cff_ai_save_settings_nonce')) { // Save AI Assistant Settings
					...
					CPCFF_AI_REQUESTS::set_selected_api_key_from_context('ai-assistant', $api_key);
					...
					CPCFF_AI_REQUESTS::set_selected_provider_from_context('ai-assistant', $provider);

					if (empty($api_key) && $provider !== $wordpress_ai) {
						// If API key is empty, reset provider and model to local
						CPCFF_AI_REQUESTS::set_selected_provider_from_context('ai-assistant', 'local');
						CPCFF_AI_REQUESTS::set_selected_model_from_context('ai-assistant', 'local');
						wp_send_json([
							'reason'  => 'empty_api_key',
							'message' => __('No API key configured for the selected provider. Falling back to the local model. Configure your key in the AI Assistant settings to use a cloud model.', 'calculated-fields-form'),
						]);
					}
					...
					CPCFF_AI_REQUESTS::set_selected_model_from_context('ai-assistant', $model);
		...
		public static function view()
		{
			...
			wp_enqueue_style('cff-ai-assistant-css', plugins_url('/css/styleassistant.ai.css', CP_CALCULATEDFIELDSF_MAIN_FILE_PATH), array(), CP_CALCULATEDFIELDSF_VERSION);
?>
			<script data-category="functional">
				...
				$ai_config_obj = [
					...
					'waiting_message_1'  => __('Still working on it...', 'calculated-fields-form'),
					'waiting_message_2'  => __('This is taking longer than usual, but please be patient...', 'calculated-fields-form'),
					...
					'placeholder_message' => __('Generate a message that ...', 'calculated-fields-form'),
					'use_list_btn'       => __('Use list', 'calculated-fields-form'),
					'copy_btn'           => __('Copy', 'calculated-fields-form'),
					'copy_message_btn'   => __('Copy message', 'calculated-fields-form'),
				...
				print 'var cff_ai_provider="' . esc_js(
					CPCFF_AI_REQUESTS::get_selected_provider_from_context('ai-assistant')
				) . '";';
				print 'var cff_ai_default_provider="' . esc_js(CPCFF_AI_REQUESTS::get_default_provider()) . '";';
				print 'var cff_ai_model="' . esc_js(
					CPCFF_AI_REQUESTS::get_selected_model_from_context('ai-assistant')
				) . '";';
			...
			<div id="cff-ai-assistant-container" style="display:none;">
				<div id="cff-ai-assistan-header" class="cff-ai-assistan-header">
					<div id="cff-ai-assistan-title" class="cff-ai-assistan-title">
						...
						<button id="cff-ai-assistant-unmount" class="button-secondary" style="display:none;"><?php esc_html_e('unload model and close', 'calculated-fields-form'); ?></button>
					...
					<div id="cff-ai-assistant-settings-container" style="cursor:initial;">
						...
						<div id="cff-ai-assitance-api-key-container" style="display:none;">
							...
							<input type="password" id="cff-ai-assistant-api-key" name="cff-ai-assistant-api-key" placeholder="<?php esc_html_e('Enter your API key here...', 'calculated-fields-form'); ?>" autocomplete="new-password" spellcheck="false" autocorrect="off" autocapitalize="off" />
				...
				<div id="cff-ai-assistant-answer-row" class="cff-ai-assistant-answer-row">
					<div class="cff-ai-assistance-first-message-local cff-ai-assistance-message cff-ai-assistance-bot-message cff-ai-assistance-loading-message">
						...
						<div id="cff-ai-assistant-loading-message">
							...
							<div id="cff-ai-assistant-errors">
								<!-- GPU Error -->
								<div id="cff-ai-gpu-error" style="display:none;">
									<h3><?php esc_html_e('WebGPU is not available in your browser/system', 'calculated-fields-form'); ?></h3>
									<p><?php esc_html_e('WebGPU is a modern graphics API for the web. To use this application, you\'ll need a browser with WebGPU support and a compatible GPU (hardware or software).', 'calculated-fields-form'); ?></p>
									<p><?php esc_html_e('Your browser may support WebGPU but no compatible GPU adapter was found. This can happen if your computer lacks a GPU, your drivers are outdated, or your browser has WebGPU enabled in a restricted mode.', 'calculated-fields-form'); ?></p>
								...
								<!-- RAM Error -->
								<div id="cff-ai-ram-error" style="display:none;">
									<h3><?php esc_html_e('Insufficient memory for local model', 'calculated-fields-form'); ?></h3>
									<p><?php esc_html_e('Local model inference requires approximately 4-8 GB of free RAM. Your browser reports less than this threshold. Consider switching to a Cloud Provider, or close other browser tabs and applications to free up memory.', 'calculated-fields-form'); ?></p>
									<p><?php esc_html_e('Note: RAM detection is only available in Chromium-based browsers (Chrome, Edge, Brave). Firefox and Safari will skip this check.', 'calculated-fields-form'); ?></p>
								</div>
							</div>
						</div>
					</div>
